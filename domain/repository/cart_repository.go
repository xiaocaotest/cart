...
func (u *CartRepository) InitTable() error {
	if !u.mysqlDb.Migrator().HasTable(&model.Cart{}) {
		if err := u.mysqlDb.Migrator().CreateTable(&model.Cart{}); err != nil {
			log.Error(err)
		}
	}
	return nil
...
func (u *CartRepository) CreateCart(cart *model.Cart) (int64, error) {
	...
	if db.RowsAffected == 0 {
		return 0, errors.New("add cart fail")
	}
	return cart.ID, nil
}
...
func (u *CartRepository) DecrNum(cartID int64, num int64) error {
	cart := &model.Cart{ID: cartID}
	db := u.mysqlDb.Model(cart).Where("nem >= ?", num).UpdateColumn("num", gorm.Expr("num - ?", num))
	if db.Error != nil {
		return db.Error
	}
	if db.RowsAffected == 0 {
